...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # print(f"bsz: {bsz}, num_generated_triples: {num_generated_triples}")
...
def get_dot_cost(pred_rel, gold_rel):
    ...
    for i in range(gold_rel.shape[-1]):
        # Set the column corresponding to the value in 'gold_rel' to 1
    ...
    cost_rel = torch.mm(pred_rel, gold_rel_logits.t())
    return cost_rel
...
class HungarianMatcher_r_h(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
    For efficiency reasons, the targets don't include the no_object. Because of this, in general,
    there are more predictions than targets. In this case, we do a 1-to-1 matching of the best predictions,
    while the others are un-matched (and thus treated as non-objects).
    ...
    def forward(self, outputs, targets):
        ...
        if self.matcher == "avg":
            # hungarian_start = time.time()
            cost = - self.cost_relation * pred_rel[:, gold_rel] - self.cost_head * 1/2 * (pred_head_start[:, gold_head_start])
            # hungarian_end = time.time()
            # print("time for hungarian: ", hungarian_end - hungarian_start)
            # print(f"cost: \n{cost}")
...
class ILPMatcher_r_h(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
    For efficiency reasons, the targets don't include the no_object. Because of this, in general,
    there are more predictions than targets. In this case, we do a 1-to-1 matching of the best predictions,
    while the others are un-matched (and thus treated as non-objects).
    ...
    def forward(self, outputs, targets):
        ...
        if self.matcher == "avg":
            cost = - self.cost_relation * pred_rel[:, gold_rel] - self.cost_head * 1 / 2 * (
                    pred_head_start[:, gold_head_start])
        elif self.matcher == "min":
            pass
